...
FirModuleInfoProvider::FirModuleInfoProvider() {
}

const std::vector<GPUA::processor::v2::PlatformInfo>& FirModuleInfoProvider::GetCodeSpecs() {
...
uint32_t FirModuleInfoProvider::GetSupportPlatformCount() const noexcept {
    return static_cast<uint32_t>(GetCodeSpecs().size());
}

GPUA::processor::v2::ErrorCode FirModuleInfoProvider::GetSupportPlatformInfo(uint32_t index, const GPUA::processor::v2::PlatformInfo*& platform_info) const noexcept {
...
GPUA::processor::v2::ErrorCode FirModuleInfoProvider::GetModuleInfo(const GPUA::processor::v2::ModuleInfo*& module_info) const noexcept {
    module_info = &ModuleInfo;
    return GPUA::processor::v2::ErrorCode::eSuccess;
}

GPUA::processor::v2::ErrorCode FirModuleInfoProvider::GetProcessorExecutionInfo(const GPUA::processor::v2::ProcessorEntryInfo*& entry_info) const noexcept {
    // three mandatory processor device functions. DO NOT MODIFY THE NEXT THREE LINES!
    ...
    // Set the number of GPU tasks of the processor. Fir has only one (see FirProcessor.cu)
    ...
    // convert task names from wstring to const wchar_t*
    // Add two entries for each additional processor task.
